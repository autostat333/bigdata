...
video_click = function(e,first)
	{
	//get clicked third menu (plug for absent video in banking)
	var third_menu_cl = document.get_elements_by_class_name('video_menu_a_clicked')[0];
	var cl_name = third_menu_cl.className.split(' ')[0];
	if (cl_name=='video_banking')
		{
		return false;
		}
	...
	window.ajax_wait.st();
	//load video
	var video_str = '<iframe width="770" height="579" src="https://www.youtube.com/embed/`video_youtube_name`" frameborder="0" allowfullscreen></iframe>';
//	alert(video_str);
	this.video_load = function(video_file_name)
		{
		var v_block = document.get_elements_by_class_name('video_canvas')[0]
		var i = 0;
		var final_str = '';
		var new_video = video_str;	
		new_video = new_video.replace('`video_youtube_name`',video_file_name);
		v_block.innerHTML = new_video;
		}
	...
	if (first=='first')
		...
		video_file_name = span_item.id.split(';')[1];
		tmp_f = span_item.id.split(';')[0];
		...
		while (end!='stop'&&i<items.length)
			...
			var video_file_name = span_item.id.split(';')[1];
			tmp_f = span_item.id.split(';')[0];
			if (tmp_f==first)
				{
				end = 'stop';
